        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 888) {
                ...
                if (orientation > 45 && orientation < 135) {
                    RxBus.get().post(ConstantValue.SCREEN_ORIENTATION_REVERSE_LANDSCAPE, "");
                    //    mActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_REVERSE_LANDSCAPE);
                 //   isLandscape = true;
                }
                else if (orientation > 135 && orientation < 225) {
                    RxBus.get().post(ConstantValue.SCREEN_ORIENTATION_REVERSE_PORTRAIT, "");
                  //  mActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_REVERSE_PORTRAIT);
                    isLandscape = false;
                }
                else if (orientation > 225 && orientation < 315) {
                    RxBus.get().post(ConstantValue.SCREEN_ORIENTATION_LANDSCAPE, "");
                    //     mActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
             //       isLandscape = true;
                }
                else if ((orientation > 315 && orientation < 360) || (orientation > 0 && orientation < 45)) {
                    //mActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
                    RxBus.get().post(ConstantValue.SCREEN_ORIENTATION_PORTRAIT, "");
                    isLandscape = false;
                }
            }
        }
